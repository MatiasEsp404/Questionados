...
public class JwtUtils {
	...
	private Claims extractAllClaims(String authorizationHeader) {
		return Jwts.parser().setSigningKey(SECRET_KEY.getBytes(StandardCharsets.UTF_8))
				.parseClaimsJws(getToken(authorizationHeader)).getBody();
	}
	...
	public String generateToken(UserDetails userDetails) {
		String token = Jwts.builder().claim(AUTHORITIES, getFirstAuthority(userDetails))
				.setSubject(userDetails.getUsername()).setIssuedAt(Date.from(ZonedDateTime.now().toInstant()))
	...
	public List<GrantedAuthority> getAuthorities(String token) {
		return AuthorityUtils
				.commaSeparatedStringToAuthorityList(Objects.toString(extractAllClaims(token).get(AUTHORITIES)));
	}
